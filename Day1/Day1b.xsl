  ...
  <xsl:template name="xsl:initial-template">
    <xsl:variable name="rotations" as="element(rotation)*">
      <xsl:iterate select="$rotations">
        ...
        <xsl:variable name="zeros" as="xs:integer*">
          <xsl:sequence select="abs(($position + .) idiv 100)">
            <!-- The number of total rotations -->
          </xsl:sequence>
          <xsl:sequence select="if ($position ne 0 and ($position + .) le 0) then 1 else 0">
            <!-- If turning the dial back reaches/passes 0 -->
          </xsl:sequence>
        </xsl:variable>
        <rotation start="{$position}" rotation="{a:integer-to-rotation(.)}" end="{$end}" zeros="{sum((0, $zeros))}"/>
        <xsl:next-iteration>
          <xsl:with-param name="position" select="$end"/>
        </xsl:next-iteration>
      </xsl:iterate>
    </xsl:variable>
    <dial zeros="{sum($rotations/@zeros)}">
